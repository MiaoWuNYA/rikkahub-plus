...
interface ManagedFileDAO {
    ...
    @Query("SELECT * FROM managed_files WHERE folder = :folder AND created_at <= :cutoffMillis ORDER BY created_at ASC")
    suspend fun listByFolderBefore(folder: String, cutoffMillis: Long): List<ManagedFileEntity>
