...
> 上游全部功能原样保留，在此基础上重点强化了 **⚡提示词前缀缓存**、**🧠记忆与长对话**、**🍺酒馆（SillyTavern）兼容**、**🛡隐私与稳定性** 四大方向。
...
- **提示词前缀缓存**：对标 DeepSeek Harness 的前缀稳定性设计，长对话 token 费用最高可降 90%
...
主流供应商（DeepSeek / Kimi / Claude 等）都提供自动前缀缓存：只要本次请求的前缀与上次完全一致即可命中，token 费用约为正常的 1/10。但聊天场景里大量内容**每轮都在变**（时间戳、最近会话、记忆、随机数、滚动摘要），前缀一动缓存全废。

本分支对标 DeepSeek Harness 的前缀稳定性设计，系统性消除这些分叉点：
...
效果：稳定聊天形态下命中率可达 90%+，长对话 token 成本接近打一折。
...
在上游基础上新增：文件操作、Shell、任务工具、计算器、数据库查询、Python 引擎（Chaquopy）、网页抓取、云南财经教务系统查询（课表/成绩/考试/公告/空教室，含独立账号设置页）；另有**系统提示词装配器**（工具选用指南 / 工作守则）。
...
- **相对 mingli2 分支**：除酒馆增强外，主要新增提示词前缀缓存、语义记忆 RAG 与上下文滚动压缩、隐私加固、云南财经教务系统工具等（酒馆部分的增量见「酒馆系统」开头的注记）
